...
class StocksService:
    ...
    async def get_stock_amount(self, stock_symbol: str) -> int:
        record_key: str = self.get_record_key(stock_symbol)
        current_stock_amount = await CacheService().get_value(record_key)
        stock_amount: int = int(current_stock_amount or 0)

        return stock_amount

    async def add_stock(self, stock_symbol: str, added_amount: int) -> int:
        ...
        async with CacheService().lock(stock_symbol) as lock:
            current_amount: int = await self.get_stock_amount(stock_symbol)
            ...
            record_key: str = self.get_record_key(stock_symbol)
            await CacheService().set_value(record_key, new_amount)
